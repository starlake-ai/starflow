...
/** A single input line rejected by a native loader.
  ...
  * @param line
  *   the 1 based line number inside that file, when the engine reports one. POSITION rejects are
  *   found by scanning a temporary table, which carries no line number, so they leave this empty.
  * @param rawLine
