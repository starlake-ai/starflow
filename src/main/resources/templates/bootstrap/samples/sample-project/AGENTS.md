...
```text
datasets/archive/starbake/      →  copy into datasets/incoming/starbake/
    customers.csv                      ↓
    orders.json                    Load (metadata/load/starbake/)
    products.json                      ↓
                               starbake.customers · starbake.orders · starbake.products
                                       ↓
...
**The sample files ship under `datasets/archive/`, not `datasets/incoming/`.**
`incoming/starbake/` holds only an empty `ack` marker, so nothing there matches
the `pattern` any of the three tables declares. Copy the files across first:
...
- `datasets/archive/starbake/` — the sample input files, as shipped
- `datasets/duckdb.db` — the warehouse, and the `audit.*` tables with it
...
# The paths below assume you copied the shipped files into datasets/incoming/ first.
